...
from .global_vars import get_args, get_tensorboard_writer, parse_user_defined_parameters, set_global_variables
from .io_utils import OSSIO, TFOSSIO, io, parse_oss_buckets
from .logger import init_logger


def initialize_easynlp(extra_args_provider=None, args_defaults={}, ignore_unknown_args=False):
    """
    看看这个初始化都干了什么
    """
    ...
    if args.mode == "train" or not args.checkpoint_dir:
        ...
        args.pretrained_model_name_or_path = parse_user_defined_parameters(args.user_defined_parameters).get(
...
def _initialize_distributed():
    ...
    if torch.distributed.is_initialized():
        ...
        torch.distributed.barrier()
        print("Init dist done. World size: {}, rank {}, l_rank {}".format(args.world_size, args.rank, args.local_rank))
...
def init_oss_io(cfg):
    if "role_arn" in cfg.buckets:
        ...
        if cfg.modelzoo_base_dir and "oss://" in cfg.modelzoo_base_dir:
            _, _, mz_hosts, mz_buckets = parse_oss_buckets(cfg.modelzoo_base_dir)
            hosts += mz_hosts
            buckets += mz_buckets
        new_io = OSSIO(access_key_id=access_key_id, access_key_secret=access_key_secret, hosts=hosts, buckets=buckets)
    io.set_io(new_io)
