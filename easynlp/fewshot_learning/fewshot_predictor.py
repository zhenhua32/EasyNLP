...
from easynlp.core.predictor import Predictor, PyModelPredictor, get_model_predictor
...
class FewshotPyModelPredictor(PyModelPredictor):
    ...
    def __init__(self, model_cls, saved_model_path, input_keys, user_defined_parameters, output_keys, **kwargs):
        self.model = model_cls.from_pretrained(
            pretrained_model_name_or_path=saved_model_path, user_defined_parameters=user_defined_parameters, **kwargs
        )
    ...
    def predict(self, in_data):
        ...
        with torch.no_grad():
            # 推理, 输出的是包含 logits 字段的字典
            predictions = self.model.forward(
                {"input_ids": in_tensor["input_ids"], "attention_mask": in_tensor["attention_mask"]}
            )
        ret = {}
        # 将输入字段也放回到结果中
...
class PromptPredictor(Predictor):
    ...
    def __init__(self, model_dir, model_cls=None, user_defined_parameters=None, *args, **kwargs):
        ...
        self.label_map = dict(zip(self.label_mapping.keys(), label_desc.split(",")))
        ...
        # 候选标签 name => input_ids
        self.label_candidates_ids = {k: str_to_ids(v, self.tokenizer) for k, v in self.label_map.items()}
    ...
    def preprocess(self, in_data):
        ...
        for record in in_data:
            ...
            try:
                ...
                self.MUTEX.release()
            assert len(label_tokens) == self.masked_length, "label length should be equal to the mask length"
...
class CPTPredictor(Predictor):
    ...
    def __init__(self, model_dir, model_cls=None, user_defined_parameters=None, *args, **kwargs):
        ...
        self.label_candidates_ids = {str_to_ids(v, self.tokenizer)[0]: k for k, v in self.label_map.items()}
        ...
        self.train_dataloader = DataLoader(
            anchor_data, batch_size=anchor_batch_size, shuffle=False, collate_fn=anchor_data.batch_fn
        )
        anchor_list = []
        anchor_labels = []
    ...
    def postprocess(self, result):
        ...
        with torch.no_grad():
            label_ids = result.pop("label_ids")
            labels = label_ids[label_ids > 0].detach().cpu().numpy()
            features = torch.nn.functional.normalize(result["features"][label_ids > 0]).detach().cpu().numpy()
        ...
        for b, pred in enumerate(predictions):
            new_results.append(
                {
                    "id": result["id"][b] if "id" in result else str(uuid.uuid4()),
                    "output": self.label_candidates_ids[pred],
                    "predictions": self.label_candidates_ids[pred],
                }
            )
        if len(new_results) == 1:
            new_results = new_results[0]
