...
class ImageTextGenerationPipeline(CLIPGPTImageTextGenerationPredictor, Pipeline):
    def format_input(self, inputs):
        ...
        return [{"idx": idx, "first_sequence": inputs[idx]} for idx in range(len(inputs))]
...
class LatentDiffusionPipeline(LatentDiffusionPredictor, Pipeline):
    def format_input(self, inputs):
        ...
        if type(inputs) == str:
            inputs = [inputs]
        return [{"idx": idx, "text": inputs[idx]} for idx in range(len(inputs))]
...
class TextMatchPipeline(TextMatchPredictor, Pipeline):
    ...
    def format_input(self, inputs):
        ...
        if type(inputs) != list:
            raise RuntimeError(
                "'TextMatchPipeline' only supports lists! \
        ...
        return [
            {"first_sequence": input_sentence_pair[0], "second_sequence": input_sentence_pair[1]}
            for input_sentence_pair in inputs
        ]
