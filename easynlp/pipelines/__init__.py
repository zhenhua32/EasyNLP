...
from .implementation import (
    Pipeline,
    TextImageGenerationPipeline,
    ImageTextGenerationPipeline,
    SequenceClassificationPipeline,
    TextImageGenerationPipeline,
    TextMatchPipeline,
    SequenceLabelingPipeline,
    MachineReadingComprehensionPipeline,
...
def pipeline(
    ...
    if task_or_model is None:
        raise RuntimeError(
            "You must specify a task or model to initialize the pipeline,  \
    ...
    if task_or_model in support_task:
        ...
        raise RuntimeError(
            "You must specify a app and corresponding model to initialize the pipeline,  \
                            invoking 'get_supported_tasks()' to get task list, \
                            invoking 'get_easynlp_model_list()' to get easynlp task model list. \
...
def get_remote_app_model_mapping() -> dict:
    ...
    try:
        ...
        if os.path.exists(local_file_path):
            logging.info(
                "Unable to get the latest version of 'easynlp_trained_model_config.json', \
                            use the old version."
            )
        else:
            raise RuntimeError
...
def get_app_model_path(model_name, disable_auto_download=False):
    ...
    if model_name in easynlp_app_mapping:
        ...
        remote_app_model_path = easynlp_app_mapping[app_model_name]["model_path"]
        if "oss://" in appzoo_base_dir:
            # If the modelzoo is put on OSS buckets, do not need to download
            app_local_path = os.path.join(appzoo_base_dir, remote_app_model_path)
